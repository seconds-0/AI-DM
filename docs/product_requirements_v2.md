...
-   **Pipeline 1: Narrator Voice (Google Live Voice API)**
    ...
    -   _Integration:_ Bot, after the WebSocket interaction with the Gemini Live API concludes for a given turn, sends relevant data (such as final user transcript, Gemini's text response, and any function call requests/responses) to the **Storyteller Genkit Flow** via HTTPS for game state updates and further processing.
...
3.  The Gemini Live API (via the WebSocket connection managed by the Bot) processes the streamed audio, interacts with the configured Gemini model (Narrator persona), and streams audio output (and/or text) back. The Bot plays any received audio.
4.  The voice interaction turn concludes (e.g., after Gemini finishes responding or user activity ends). The Gemini Live API (via the WebSocket) will have provided the Bot with the user's input transcription, Gemini's textual response, and potentially other data like function call details.
5.  Bot POSTs the collected data from the voice interaction (final user transcript, Gemini's text response, etc.) to the **Storyteller Genkit Flow**.
